...
<div class="fieldcontain ${hasErrors(bean: enrollmentInstance, field: 'regClass', 'error')} required">
	...
	<g:select id="regClass" name="regClass.id" from="${uk.ac.shu.webarch.eregister.Class.list()}" optionKey="id" required="" value="${enrollmentInstance?.regClass?.id}" class="many-to-one"/>
</div>
